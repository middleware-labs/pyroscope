...
func (s *Storage) initExemplarsStorage(db ClickHouseDBWithCache) {
	...
	fmt.Println("initExemplarsStorage step 1...")
	e := exemplars{
		logger:  s.logger,
		config:  s.config,
		metrics: s.metrics,
	...
	s.exemplars = &e
	s.tasksWG.Add(1)
	fmt.Println("initExemplarsStorage step 2...")
	go func() {
		...
		fmt.Println("initExemplarsStorage step 3...")
		for {
			select {
			default:
			...
			select {
			case <-s.stop:
				fmt.Println("initExemplarsStorage case 1...")
				e.logger.Debug("flushing batches queue")
				e.flushBatchQueue()
				return

			case <-batchFlushTicker.C:
				fmt.Println("initExemplarsStorage case 2...")
			...
			case batch, ok := <-e.batches:
				fmt.Println("initExemplarsStorage case 3...")
				if ok {
					e.flush(batch)
				}

			case <-retentionTicker.C:
				fmt.Println("initExemplarsStorage case 4...")
...
func (e *exemplars) flushCurrentBatch() {
	fmt.Println(" flushCurrentBatch e.currentBatch.entries", e.currentBatch.entries)
...
func (b *exemplarsBatch) writeExemplarToDB(db ClickHouseDB, e *exemplarEntry) error {
	...
	// fmt.Println("writeExemplarToDB keval.....")
