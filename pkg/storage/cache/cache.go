...
func printInterface(value interface{}) {
	...
	case reflect.Ptr:
		...
		printInterface(v.Elem().Interface())
	...
	case reflect.Struct:
		...
		for i := 0; i < v.NumField(); i++ {
			field := v.Field(i)
			fmt.Printf("\t%v: ", v.Type().Field(i).Name)
			printInterface(field.Interface())
		}
		fmt.Println("}")

	case reflect.Slice, reflect.Array:
		fmt.Printf("%v [", v.Type().Name())
		for i := 0; i < v.Len(); i++ {
			if i > 0 {
				fmt.Print(", ")
			}
			printInterface(v.Index(i).Interface())
		}
		fmt.Println("]")
...
func (cache *Cache) saveToDisk(key string, val interface{}) error {
	fmt.Println("savetodisk key ...", key)
	fmt.Println("savetodisk val ...", val)
	...
	bytes := b.Bytes()
	fmt.Println("typeof ....", reflect.TypeOf(bytes))
	q := fmt.Sprintf("INSERT INTO cache (key, val) VALUES ('%s', '%s')", key, hex.EncodeToString(bytes))
